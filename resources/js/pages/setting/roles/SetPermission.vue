<template>
    <div class="col-md-12">
        <div class="row">
            <div class="col-md-5">
                <div class="panel">
                    ...
                    <!-- FORM UNTUK MENAMBAHKAN ROLE KE USES -->
                </div>
            </div>
            <div class="col-md-7">
                <div class="panel">
        ...
        <input type="checkbox" class="minimal-red" :key="index" :value="row.name" :checked="role_permission.findIndex(x => x.name == row.name) != -1" @click="addPermission(row.name)"> {{ row.name }} <br :key="'row' + index">
...
export default {
    ...
    created() {
        // KETIKA COMPENENT DI-LOAD, MAKA KITA AKAN ME-REQUEST 3 DATA BERIKUT
    ...
    methods: {
        // LOAD SEMUA FUNGSI YANG ADA DI MODULE STORE USE
        ...mapActions('user', [
            'getUserLists',
            'getRoles',
            'getAllPermission',
        ...
        checkPermission() {
            ...
            // KIRIM PERMINTAAN KE BACKEND
            this.getRolesPermission(this.role_selected).then(() => {
